...
class ReActRunner(BaseLlmRunner):
    ...
    def run_prompt(self):
        ...
        tools = [repl_tool, get_codeql_results, get_spotbugs_results, get_ast_from_code]  # todo ast does not work. The other one where code was provided worked better. Potentially pre-load this data? + add scat retriavl tool
        ...
        with get_openai_callback() as cb:
            ...
            # Make calls
            start = time.time()
            llm_response = agent_executor.invoke({"code": code}, verbose=True)
            end = time.time()
